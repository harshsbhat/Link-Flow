'use client';
import React from "react";
import { addDays, differenceInDays, formatISO9075, parseISO } from "date-fns";
import { CartesianGrid, Line, LineChart, ResponsiveContainer, Tooltip, XAxis, YAxis } from "recharts";

export default function Chart({ data }) {
  // Check if data is an array and contains at least one element
  if (!Array.isArray(data) || data.length === 0) {
    return <div>No data available.</div>;
  }

  // Find the first object in data that is not empty
  const firstDataItem = data.find(item => Object.keys(item).length > 0);

  // Check if a valid firstDataItem was found
  if (!firstDataItem) {
    return <div>No valid data item found.</div>;
  }

  // Retrieve the keys from the first data item
  const keys = Object.keys(firstDataItem);

  // Find the key that is not 'date'
  const xLabelKey = keys.find(key => key !== 'date');

  // Check if xLabelKey is defined
  if (!xLabelKey) {
    return <div>No valid xLabelKey found.</div>;
  }

  // Prepare data without gaps
  const dataWithoutGaps = [];
  data.forEach((value, index) => {
    const date = value.date;
    const newDataItem = { date };
    newDataItem[xLabelKey] = value[xLabelKey] || 0;
    dataWithoutGaps.push(newDataItem);

    // Calculate and fill gaps between dates with missing data
    const nextDate = data?.[index + 1]?.date;
    if (date && nextDate) {
      const daysBetween = differenceInDays(parseISO(nextDate), parseISO(date));
      if (daysBetween > 0) {
        for (let i = 1; i < daysBetween; i++) {
          const dateBetween = formatISO9075(addDays(parseISO(date), i)).split(' ')[0];
          dataWithoutGaps.push({ date: dateBetween, [xLabelKey]: 0 });
        }
      }
    }
  });

  return (
    <div>
      <ResponsiveContainer width="100%" height={200}>
        <LineChart
          width={730}
          height={250}
          data={dataWithoutGaps}
          margin={{ top: 5, right: 30, left: 20, bottom: 5 }}
        >
          <CartesianGrid horizontal={false} strokeWidth="2" stroke="#f5f5f5" />
          <XAxis dataKey="date" axisLine={false} tickLine={false} tickMargin={10} tick={{ fill: '#aaa' }} />
          <YAxis axisLine={false} tickLine={false} tickMargin={10} tick={{ fill: '#aaa' }} />
          <Tooltip />
          <Line type="monotone" dataKey={xLabelKey} stroke="#09f" strokeWidth="4" />
        </LineChart>
      </ResponsiveContainer>
    </div>
  );
}
